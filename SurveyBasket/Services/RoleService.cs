...
public class RoleService(ApplicationDbContext context , RoleManager<ApplicationRole> roleManager) : IRoleService
{
    ...
    public async Task<IEnumerable<RoleResponse>> GetAllAsync(bool? includedDisabled = false, CancellationToken cancellationToken = default)  => await _roleManager.Roles
        .Where(x => !x.IsDefault && (!x.IsDeleted ||(includedDisabled.HasValue && includedDisabled.Value)))
    ...
    {
        var roleIsExits = await _roleManager.RoleExistsAsync(request.Name);
